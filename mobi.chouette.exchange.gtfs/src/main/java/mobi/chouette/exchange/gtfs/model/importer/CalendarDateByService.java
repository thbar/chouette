...
public class CalendarDateByService extends IndexImpl<GtfsCalendarDate>
    ...
    protected GtfsCalendarDate build(GtfsIterator reader, Context context) {
        ...
        value = array[i++];
        testExtraSpace(FIELDS.date.name(), value, bean);
        if (value == null || value.trim().isEmpty()) {
            if (withValidation)
        ...
        value = array[i++];
        testExtraSpace(FIELDS.exception_type.name(), value, bean);
        if (value == null || value.trim().isEmpty()) {
            if (withValidation)
    ...
    public boolean validate(GtfsCalendarDate bean, GtfsImporter dao) {
        ...
        if (bean.getDate() != null && bean.getServiceId() != null) {
			result = hashCodes.add(bean.getServiceId() + "#" + bean.getDate().atStartOfDay(ZoneId.systemDefault()).toEpochSecond());
        }
        if (!result) {
