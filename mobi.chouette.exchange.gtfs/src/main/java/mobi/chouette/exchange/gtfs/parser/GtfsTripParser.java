...
//	}
    ...
    protected void convert(Context context, GtfsTrip gtfsTrip, VehicleJourney vehicleJourney) {
        ...
        StopPoint arrivalStopPoint = null;
        StopPoint departureStopPoint = null;

        if (vehicleJourney.getJourneyPattern() != null) {
            arrivalStopPoint = vehicleJourney.getJourneyPattern().getArrivalStopPoint();
            departureStopPoint = vehicleJourney.getJourneyPattern().getDepartureStopPoint();
        }

        if (gtfsTrip.getTripHeadSign() != null) {
            vehicleJourney.setPublishedJourneyName(gtfsTrip.getTripHeadSign());
        } else {
            if ((arrivalStopPoint != null)) {
                vehicleJourney.setPublishedJourneyName(arrivalStopPoint.getContainedInStopArea().getName());
            }
        }
        ...
        if (gtfsTrip.getTripShortName() != null) {
            ...
            if (departureStopPoint != null && arrivalStopPoint != null) {

                StringBuilder sb = new StringBuilder();
                vehicleJourney.setPublishedJourneyIdentifier(
                        sb.append(departureStopPoint.getContainedInStopArea().getName())
                                .append(PUBLISHED_JOURNEY_IDENTIFIER)
                                .append(arrivalStopPoint.getContainedInStopArea().getName())
                                .append(PUBLISHED_JOURNEY_IDENTIFIER)
                                .append(vehicleJourney.getVehicleJourneyAtStops().get(0).getDepartureTime())
                                .toString());
            }
        }
