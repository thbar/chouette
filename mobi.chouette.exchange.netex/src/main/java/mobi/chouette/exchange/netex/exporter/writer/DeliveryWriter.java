...
public class DeliveryWriter extends AbstractWriter{
	...
	public static void write(Writer writer, ExportableData data ) throws IOException, DatatypeConfigurationException 
	{
		...
		writer.write("  <PublicationTimestamp>"+timeFormatter.format(now)+"</PublicationTimestamp>\n");
		...
		writer.write("    <RequestTimestamp>"+timeFormatter.format(now)+"</RequestTimestamp>\n");
		...
		writer.write("    <!-- =========================================== -->    \n");   
		writer.write("    <CompositeFrame version=\"1\" created=\""+timeFormatter.format(line.getNetwork().getVersionDate())+"\" " +
