        ...
        try {
            for (int i = 0; i < routeStopCount; i++) {
                ...
                scheduledStopPoint.setObjectId("T:ScheduledStopPoint:" + route.objectIdSuffix() + "-"
                        + i);
                scheduledStopPoint.setContainedInStopAreaRef(new SimpleObjectReference<>(quays.get(i % quayCount)));
    ...
    private VehicleJourney vehicleJourney(Calendar calendar,
        ...
        try {
            ...
            for (int i = 0; i < journeyPattern.getStopPoints().size(); i++) {
                ...
                vjas.setArrivalTime(LocalDateTime.ofInstant(calendar.toInstant(), ZoneId.systemDefault()).toLocalTime());
                vjas.setDepartureTime(LocalDateTime.ofInstant(calendar.toInstant(), ZoneId.systemDefault()).toLocalTime());
